...
public:
    int minPathSum(vector<vector<int> >& obstacleGrid) {
        ...
        vector< vector<uint64_t> > steps; //存放到
        steps.resize(m);
        ...
        steps[0][0] = obstacleGrid[0][0];

        for(int i = 1; i < m; i++) 
        {
            steps[i][0] = steps[i-1][0] + obstacleGrid[i][0];
        }

        for(int j = 1; j < n; j++) 
        {
            steps[0][j] = steps[0][j-1] + obstacleGrid[0][j];
        }
        ...
        for(size_t i = 1; i < m; i++)
        {
            ...
            {
                steps[i][j] =   std::min(steps[i-1][j] , steps[i][j-1]) + obstacleGrid[i][j] ;
            }
        }
